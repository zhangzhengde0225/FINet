...
class LoadImages:  # for inference
	def __init__(self, path, img_size=640, roi=None):
		self.roi = roi
	...
	def __next__(self):
		...
		# Padded resize
		img, _, _, recover_xy = letterbox(img0, new_shape=self.img_size, roi=self.roi)
		# cv2.imshow(f'x', img)
		# cv2.waitKey(0)
		# print(type(img))
		# Convert
		...
		return path, img, img0, self.cap, recover_xy
...
class LoadImagesAndLabels(Dataset):  # for training/testing
	...
	def __getitem__(self, index):
		...
		if self.mosaic:
			# Load mosaic
			img, labels = load_mosaic(self, index)  # (608, 608, 3)  (n, 5) n是1-4这样可以保证数据都是有目标的。
			shapes = None
		...
		if self.augment:
			# Augment imagespace
			if not self.mosaic:  # not True, 没进去
			...
			# Augment colorspace
			augment_hsv(img, hgain=hyp['hsv_h'], sgain=hyp['hsv_s'], vgain=hyp['hsv_v'])  # 色域变换：色调、饱和度和亮度
