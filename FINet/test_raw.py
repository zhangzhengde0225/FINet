...
def test(
	...
	for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
		...
		with torch.no_grad():
			...
			# inf_out:  tensor (4, 13167, 25)
			# train_out: list len 3, torch.Size([4, 3, 44, 76, 25]), torch.Size([4, 3, 22, 38, 25]), torch.Size([4, 3, 11, 19, 25])]
	...
	if len(stats) and stats[0].any():
		...
		# print(f'\np: {p} {p.shape} r: {r} 1: {f1} ap: {ap50} fap_class: {ap_class}')
	else:
		nt = torch.zeros(1)
	...
	print(f'{"all":>20}{seen:>12}{nt.sum():>12}{mp:>12.3f}{mr:>12.3f}{map50:>12.3f}{map:>12.3f} F1: {mf1:.3f}')
	if show_each_cls:
		for cls in ap_class:
			name = names[cls]
			print(
				f'{name:>20}{seen:>12}{nt[cls]:>12}{p[cls]:>12.3f}{r[cls]:>12.3f}'
				f'{ap50[cls]:>12.3f}{ap[cls]:>12.3f} F1: {f1[cls]:.3f}')
	...
	if opt.task in ['val', 'test']:  # run normally
		test(opt,
			opt.source,
			 ...
			 opt.augment,
			 opt.verbose,
			 show_each_cls=opt.show_each_cls)
