...
def train(hyp, opt, device, tb_writer=None):
	...
	for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
		...
		if rank in [-1, 0]:
			...
			if opt.save_interval:  # 如果不为None
				assert isinstance(opt.save_interval, int) and opt.save_interval > 0  # save model every n epochs
				if epoch % opt.save_interval == 0:
					...
					torch.save(ckpt, f'{wdir}/epoch{epoch:0>3}.pt')
					del ckpt
...
def get_opt():
	...
	parser.add_argument('-p', '--policy', type=str, default=None, help='preset train policy')
...
def policy(opt):
	...
	if p == 'p1':
		plc = dict(
			desc='seyolov5 resume from ep100, to ep300',
			# resume='runs/se_m_ep99_fogged/weights/last.pt',
			trainset_path='~/datasets/insulator/SFID',
			epochs=300,
		)
	elif p == 'p2':
		plc = dict(
		...
		plc = dict(
			desc='seyolov5 retrain, 每隔2代保存一次',
			trainset_path='~/datasets/insulator/SFID',
		...
		plc = dict(
			desc='yolov5 retain, 每隔2代保存一次',
			trainset_path='~/datasets/insulator/SFID',
			weights='runs/m_ep185/weights/last.pt',
			epochs=300,
			not_use_SE=True,
			save_interval=2,
		)
	...
	opt.resume = plc.get('resume', opt.resume)
	opt.epochs = plc.get('epochs', opt.epochs)
	opt.trainset_path = plc.get('trainset_path', opt.trainset_path)
	opt.not_use_SE = plc.get('not_use_SE', opt.not_use_SE)
	opt.save_interval = plc.get('save_interval', opt.save_interval)
	opt.weights = plc.get('weights', opt.weights)

	opt.trainset_path = os.path.expanduser(opt.trainset_path)  # ~扩展为实际路径
